...
exception.
*****************************************************************************************/
stringT CMD5Checksum::GetMD5( LPCTSTR lpszFilePath, bool bHex )
{
	//open the file as a binary file in readonly mode, denying write access 
	FILE *f=NULL;
	f = fopen( lpszFilePath, _T("rb") );
	if ( f )
	{
		stringT strMD5 = GetMD5( f ,bHex);
		fclose( f );
...
*****************************************************************************************/
stringT CMD5Checksum::GetMD5( FILE * f ,bool bHex)
{
	try
	{
		...
		while ((nLength = fread( Buffer,sizeof(BYTE), nBufferSize, f )) > 0 )
		{
...
*****************************************************************************************/
stringT CMD5Checksum::GetMD5(BYTE* pBuf, UINT nLength, bool bHex )
{
	//entry invariants
...
*****************************************************************************************/
stringT CMD5Checksum::Final(bool bHex)
{
	//Save number of bits
	BYTE Bits[8];
	...
	if (bHex) {
		for (int i = 0; i < nMD5Size; i++)
		{
			TCHAR szTemp[3] = { 0 };
			if (lpszMD5[i] == 0) {
				sprintf(szTemp, _T("00"));
			}
			else if (lpszMD5[i] <= 15) {
				sprintf(szTemp, _T("0%x"), lpszMD5[i]);  //Str.Format("0%x",lpszMD5[i]);
			}
			else {
				sprintf(szTemp, _T("%x"), lpszMD5[i]); //Str.Format("%x",lpszMD5[i]);
			}

			assert(strlen(szTemp) == 2);
		...
		strMD5.assign((const char*)lpszMD5, 16);
		assert(strMD5.size() == 16);
	}
	return strMD5;
}
